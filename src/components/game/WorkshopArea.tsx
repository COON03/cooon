...
const WorkshopSlot: React.FC<{ weapon: Weapon | null }> = ({ weapon }) => {
    return (
        <Card className="h-40 flex items-center justify-center bg-black/20 border-dashed">
            {weapon ? (
                <div className="text-center p-2">
                    <WeaponIcon type={weapon.type} />
                    <p className="font-bold mt-2 truncate">{weapon.name}</p>
                    <p className="text-xs text-muted-foreground">공: {weapon.attack} / 속: {weapon.speed}</p>
                </div>
            ) : (
...
const WorkshopArea: React.FC<WorkshopAreaProps> = ({ slots, onCombine, onClear }) => {
  return (
    <div className='flex flex-col h-full'>
      <h2 className="text-2xl font-headline font-semibold mb-4 text-primary-foreground/90">대장간 (무기 조합)</h2>
      <div className="flex-grow grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] items-center gap-4">
